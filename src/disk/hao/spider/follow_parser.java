...
public class follow_parser implements Parser{
    private int limit = 24;
    public ArrayList<String> get_urls(String userid){
        ArrayList<String> follow_urls = new ArrayList<>();
        try{
...
//            String url_share = linkFormat.url_share_format(config.share_link,userid,limit,i);
                String url_follow = linkFormat.url_user_format(config.follow_link,userid,limit,i);
                follow_urls.add(url_follow);
            }
        }catch(IOException e){
            e.printStackTrace();
        }
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        follow_parser sharelistSpider = new follow_parser();
